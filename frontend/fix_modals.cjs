...
filesToFix.forEach(file => {
    ...
    while (true) {
        ...
        if (checkPortal !== -1) {
            ...
            if (textBetween.trim() === 'createPortal(' || textBetween.trim() === 'return createPortal(') {
                isWrapped = true;
            }
        }
        ...
        if (endIdx !== -1) {
            let beforeDiv = content.substring(divStart - 20, divStart);
